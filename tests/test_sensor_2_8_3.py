...
def test_agent_2_8_3_fixed_sensor_values() -> None:
    ...
    cpu_payload = {
        "cpu": {"usage_percent": 14.8, "temperature_c": 35.3, "frequency_mhz": 1500}
    }
    assert descriptions["cpu_usage"].attributes_fn(cpu_payload) == {
        "temperature": 35.3,
        "frequency": 1500,
    }
    ...
    assert descriptions["network_status"].attributes_fn(STATUS) == {
        "interface": "eth0",
        "link_speed": 1000,
        "download": 5531,
        "upload": 8072,
...
def test_reserved_enum_states_map_to_unavailable() -> None:
    descriptions = {description.key: description for description in CORE_SENSORS}
    assert (
        descriptions["status"].value_fn({"health": {"status": "unavailable"}}) is None
    )
    assert (
        descriptions["network_status"].value_fn({"network": {"status": "unknown"}})
        is None
    )
...
def _smart_entity(row: dict[str, object]) -> MonitorSuiteSmartStatusSensor:
    ...
    entity.coordinator = SimpleNamespace(
        last_update_success=True, data={"smart": {"devices": [row]}}
    )
